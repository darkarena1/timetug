...
# Existing assets of a draft: `gh api repos/../releases/<id>/assets` returns $FAKE_ASSETS (default empty list).
case "$*" in *"/assets" ) [ "${1:-}" = api ] && { cat "${FAKE_ASSETS:-/dev/null}" 2>/dev/null || echo '[]'; } ;; esac
exit 0
STUB
chmod +x "$T/bin/gh"
...
echo "=== draft re-run: same-name asset deleted before POST; other names kept ===" >&2
reset
echo '[{"id":900,"name":"a.dmg"},{"id":901,"name":"other.txt"}]' > "$T/assets.json"
FAKE_ASSETS="$T/assets.json" $U v1.1.0 deadbeef a.dmg a.dmg.sha256 >/dev/null
[ "$(grep -c 'method DELETE' "$FAKE_LOG")" = 1 ] || fail "one delete: $(cat "$FAKE_LOG")"
grep -q 'method DELETE repos/o/r/releases/assets/900' "$FAKE_LOG" || fail "delete id 900"
[ "$(grep -n 'DELETE' "$FAKE_LOG" | cut -d: -f1)" -lt "$(grep -n 'POST.*name=a.dmg -H' "$FAKE_LOG" | cut -d: -f1)" ] || fail "delete before post"
reset
$U v1.1.0 deadbeef a.dmg >/dev/null
grep -q DELETE "$FAKE_LOG" && fail "delete without matching asset"
echo "PASS" >&2

echo "=== asset name is percent-encoded in the upload URL ===" >&2
reset
printf x > "$T/a+b c.zip"
$U v1.1.0 deadbeef "$T/a+b c.zip" >/dev/null
grep -qF 'assets?name=a%2Bb%20c.zip' "$FAKE_LOG" || fail "encoding: $(cat "$FAKE_LOG")"
echo "PASS" >&2
