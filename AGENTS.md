...
- `.github/workflows/release.yml`: the repo has GitHub IMMUTABLE RELEASES (a published release's assets and tag are locked), so the owner drafts a release for a NEW `v*` tag in the GitHub UI, saves it as a DRAFT (never clicks Publish), then runs the workflow (`workflow_dispatch` with `tag`, or push the tag). It fails at once if the tag is already published (`scripts/release/release-state.sh`), builds the tag's commit (must be on `master`; needs approval via the `release` environment) with `scripts/ci/build-release.sh`; signed and notarized DMG when the Apple secrets exist, otherwise an unsigned prerelease DMG (`scripts/release/`). `scripts/release/upload-release-assets.sh` uploads the DMG, `.sha256` and Sparkle zip to the draft and publishes it (creating the tag at the built commit if missing; with no draft it creates the release), then the appcast item is added (a `-` tag goes to the beta channel; unsigned never does). A tag push checks out `github.ref` (qualified, so a same-named branch cannot shadow it) and a `Validate the tag` step rejects a bad tag (e.g. `V1.2.3`) before any build. Fails early if the Apple secrets exist without `SPARKLE_PRIVATE_KEY`. Tag `v1.2.0` is burned (published empty); use `v1.2.1` or higher. Process and secrets: `docs/release.md`; rationale: ADR 0007, 0008, 0011.
