...
## CI and releases
- `.github/workflows/ci.yml`: on push to `master` and every PR (build and tests only; PRs get no signing, no secrets, and nothing that can enter the update feed). Jobs: `core` (Core tests + EventKitSource build + inference package tests), `app` (XcodeGen + app tests, uploads the `.xcresult` on failure), `dmg` (unsigned DMG), `core-linux` (allowed to fail; proves Core portability), `connectors-linux` (allowed to fail; proves the connector library builds and passes on Linux, swift:6.2 because swift-crypto needs a newer toolchain than `core-linux`'s image).
