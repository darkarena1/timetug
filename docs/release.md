...
## Cut a release
The repository has GitHub **immutable releases** enabled: once a release is published, its assets and tag are locked and nothing can be uploaded to it. Assets can only be attached while the release is a draft, so the workflow does the publishing, not you.

1. Make sure `master` is green and `docs/manual-tests/macos-checklist.md` has been run.
2. In GitHub, draft a new release (Releases > Draft a new release). Create a NEW tag that starts with `v`, for example `v1.2.1`, on `master`, and write the title and notes. Click **Save draft**. **Never click Publish**: a published release is immutable and empty, and its tag can never be used again. Drafts fire no release events, so saving does nothing by itself.
3. Run the `Release` workflow: Actions > Release > Run workflow with the `tag` input (for example `v1.2.1`), or push the `v*` tag. Either way the commit must be on `master`. The workflow first validates the tag (`compute-versions.sh stable`, so `V1.2.3` fails before any build) and runs `scripts/release/release-state.sh`; if the tag is already published it fails at once ("already published and immutable"). For a tag push it checks out `github.ref` (the tag); for a manual run it builds the selected commit, or the tag's commit if the tag exists. It waits for your approval in the `release` environment, builds, signs and notarizes the app and DMG, and `scripts/release/upload-release-assets.sh` uploads `TimeTug-<version>.dmg`, its `.sha256` and the Sparkle zip to your draft and publishes it. Your title and notes are not touched; the tag is created at the built commit if it did not exist. Only after publishing does it add the item to the appcast (see "Publishing the stable update"). The version is the tag without the `v`.
4. The release is a draft (invisible) for the roughly 30 minutes the build takes; it appears with its assets when published, and the appcast item follows. Do not announce it until the workflow is green.

If there is no draft for the tag, the workflow creates the release itself with generated notes and the DMG SHA-256.

A tag with a `-` suffix (for example `v1.2.3-rc1`) is published as a prerelease and goes to the `beta` channel of the appcast, not the stable feed. If the Apple secrets are missing, the workflow uploads an unsigned DMG, publishes a prerelease and never adds it to the feed.

**Burned tag `v1.2.0`.** An earlier version of the workflow ran on `release: published` and tried to upload to a release the owner had already published; it failed with `HTTP 422: Cannot upload assets to an immutable release`. `v1.2.0` is published, immutable and empty, and its tag can never be reused. The next version is `v1.2.1` or higher.
...
Once the release is published it is immutable, so the workflow cannot be re-run for that tag (it fails early with "already published"). A failure BEFORE the publish step leaves the draft intact and the workflow can simply be re-run. A failure AFTER it (the appcast step) is recovered by hand:
...
- If the release itself is wrong (bad assets), it cannot be fixed in place: delete nothing by hand, cut a new version.
...
- **The Release run fails early with "release <tag> is already published and immutable".** That tag was already published (by hand or by an earlier run). Its assets are locked and it cannot be reused. Draft a new release for a higher version and run the workflow with that tag. `scripts/release/release-state.sh <tag>` prints `none`, `draft` or `published`.
