...
- Last known counts: Core 189, AppleIntelligenceInference 7, app 131.
...
- **Fix A (done, commit `1c8384d`)**: exact/rule/user merges happen first with NO size cap; model verdicts are votes between groups (same must outnumber soft different votes; at most 4 clusters per result group); hard blocks (rules-separate, user "not the same" lessons) always block.
