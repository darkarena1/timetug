...
4. When the owner cuts a GitHub Release on a `v*` tag (publishing it, or saving a draft and running the workflow manually), CI builds the full DMG as before, attaches it and the stable Sparkle update to that release, and adds it to the feed.
...
### `release.yml` (stable)
Trigger: `release: published` for a tag starting with `v` (`beta-*` releases are ignored), and `workflow_dispatch` with a `tag` input. There is no tag-push trigger (publishing creates the tag and would fire both). The owner either publishes the release in the GitHub UI (the workflow then attaches the assets to it) or saves a draft, which fires no event, and runs the workflow manually (it uploads to the draft and publishes it). GitHub immutable releases lock a published release's assets and tag; that setting is currently OFF, and if it is turned on only the draft flow can work. Early in the run `scripts/release/release-state.sh` (`none|draft|published|published-immutable`) fails the run if the release is published and immutable.
...
- With all Apple secrets: sign, notarize and staple the app and the DMG, build the Sparkle zip from the stapled app and EdDSA-sign it, then attach the DMG, its `.sha256` and the zip (`scripts/release/upload-release-assets.sh`: a draft gets an upload then a publish, creating the tag at the built commit if missing; a published release gets `gh release upload --clobber`; with no release one is created with generated notes). The owner's title and notes are not overwritten. Then add the stable appcast item. A `-` suffix tag (for example `v1.2.3-rc1`), or a release the owner marked as a pre-release in the GitHub UI, goes to the beta channel.
- Without them: upload an unsigned DMG, mark the release a prerelease and never add it to the feed.
- In the publish flow the release is visible without assets for the roughly 30 minutes the build takes; in the draft flow it stays invisible until complete. The appcast item appears only after the assets are attached.
- Re-running for an existing, non-immutable tag builds new bytes with a new build number, re-uploads with `--clobber` and replaces the tag's appcast item (same enclosure URL). Recovery is in `docs/release.md`.
