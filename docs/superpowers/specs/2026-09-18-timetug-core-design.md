...
- **Settings** (SwiftUI, stored in `UserDefaults` as the Core Codable model): lead time (0 labelled "at start"); per-calendar tug opt-in (the "Tug" checkbox) and separate dropdown visibility (default on); takeover filters (video-link only default off; skip solo and skip declined default on; all-day events never take over); "Skip all-day events" (default on, in the Calendars section) only hides all-day events from the day list; menu bar display mode; launch at login; a "Test tug" button. Settings panes: General (menu bar mode, launch at login, About), Calendars, Tug Rules. Tug and "Show in list" are coupled: turning Tug on shows the calendar in the list, and hiding a calendar turns its Tug off.
