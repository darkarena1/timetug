...
- **Left click:** popup (NSPopover, sized to its content, max about 520 pt) with source problems at top, a header (weekday, date, meetings left, settings gear), one card per event today (calendar color, time, Now pill and progress for the current one, countdown chip and a provider-labelled Join button for the next one, finished ones in secondary text) and a footer showing the lead time. **Right click:** Settings, Quit.
- **Overlay:** one borderless window per display at a high window level (covers full-screen apps), tracks display connect/disconnect. Every display gets a blurred navy backdrop (solid navy under Reduce Transparency) and a quick fade-in. Primary display: puppy hero, live countdown ("Starts in" m:ss, orange under a minute, "Starting now", then "Started N min ago"), title, details (time range, calendar, people count), large **Join** (if link), **Snooze** menu (1/5/10 min, capped), **Dismiss**. Other displays: mirrored countdown and title, no controls. Return joins (dismisses without a link), Esc dismisses, 1/5/0 snooze; overlay takes key focus so stray keystrokes do not reach background apps; VoiceOver announces the meeting. Respects reduce-motion and accessibility labels.
- **Settings** (SwiftUI, stored in `UserDefaults` as the Core Codable model): lead time (a picker; 0 labelled "At start"); per-calendar tug opt-in (the "Tug" checkbox; calendars are listed grouped by account) and separate dropdown visibility (default on); takeover filters (video-link only default off; skip solo and skip declined default on; all-day events never take over); "Skip all-day events" (default on, in the Calendars section) only hides all-day events from the day list; menu bar display mode; launch at login; a "Test tug" button. Settings panes: General (launch at login, menu bar mode, Appearance last), Calendars, Tug Rules. About TimeTug is in the status item's right-click menu (own window), not in Settings. Tug and "Show in list" are coupled: turning Tug on shows the calendar in the list, and hiding a calendar turns its Tug off.
