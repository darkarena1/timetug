...
Shrinking the bridge by making `TimeTugCore` use the library's native forms directly (planned as **Phase 2.5**, after this phase lands). The goal is a minimal bridge, not no bridge: it keeps only what is TimeTug-specific (the wrapper carrying `sourceID` and merge/display state, and translating change and error types), while times, dates and all-day events are used as the library provides them, with no per-refresh conversions. It needs `CalendarCore` to be dependency-free, by moving the OAuth code that uses swift-crypto (PKCE, `OAuthClient`, `AccessTokenProvider`) into its own generic `CalendarOAuth` module, so Core does not pick up swift-crypto, and zone-aware all-day handling inside Core, write support (Phase 3), Microsoft (Phase 4), CalDAV/iCloud-direct, webhook push, iOS, any UI beyond the Accounts pane and the existing Calendars tab. Shipping a Google OAuth client inside release builds is a separate follow-up (see "OAuth client configuration").
...
- Library layering: put as much cross-platform functionality as possible in the generic, dependency-light library modules. OS-specific code goes only in separate, optional adapter modules that implement a library protocol (as `CredentialStore` does), so a user of the library can always supply their own implementation. The library ships no adapter that its generic modules depend on.
...
TimeTugCore (pure)            CalendarCore / GoogleCalendar (portable library)
   ...
   TimeTugCalendarEvent          mapper + adapter       CalendarEvent, CalendarDescriptor
     ...
     CalendarStore            Apps/macOS (composition root)   EventKitSource (Mac-only kind + source)
                                reconciler, Accounts pane, wiring of the adapters below
```
...
- Add a portable all-day helper to `CalendarCore` (`AllDay`, Foundation only): given the first day and the exclusive end day as calendar dates (year, month, day) plus a `TimeZone`, it returns the canonical `start`/`end` instants (start of day of each date in that zone, so a DST-missing midnight still yields a valid instant). It also offers the reverse, `dates(start:end:in:)`, and the "inclusive end date to exclusive" adjustment for providers that report the last covered day. Connectors call it instead of hand-rolling date math; the bridge uses the reverse to convert to TimeTug's form. `GoogleEventMapper` switches to it, with its existing tests unchanged.
- Add a reusable conformance check to `CalendarTestSupport`: `AllDayConformance.check(_ event:)` asserts the canonical invariants (`isAllDay` implies non-nil `timeZone`, `start` is a midnight in that zone, `end > start` and is a midnight in that zone). Every connector's mapper tests run their all-day fixtures through it, so a new connector cannot silently emit a provider-native form.
...
1. **Native** (provider-specific): Google sends `date` strings and the calendar's time zone; EventKit sends floating device-local dates with an `endDate` that is typically the last day's end (23:59:59), not the next midnight. The connectors still to come differ again: iCloud/CalDAV and IMAP invites use iCalendar `VALUE=DATE` values with an exclusive end and no zone (the connector supplies the calendar's or the device's zone), and Microsoft Graph sends `isAllDay` with midnight datetimes in a separately named zone (often UTC or a Windows zone id that must be mapped to IANA) and an exclusive end.
...
3. **TimeTug native** (`TimeTugCalendarEvent`, documented on the type; an interim form for this phase, so that Core is unchanged. Phase 2.5 makes Core read the canonical form directly and deletes this conversion, which is why it is isolated in the one `AllDay` reverse helper): `start` is the **device-local** midnight of the first day, `end` the device-local midnight after the last day (exclusive). All-day events in TimeTug are dates in the user's own calendar, so they fall on the same day the user sees in their calendar app, regardless of the source's zone.

Every future connector (iCloud/CalDAV, IMAP, Microsoft) owns exactly one job here: map its native form to canonical using the `AllDay` helper and pass `AllDayConformance`. Nothing else in the library, bridge or app changes to support a new provider's all-day quirks.

Conversions:

- **Connector, native to canonical.** Google already does this (`GoogleEventMapper`: `date` parsed in the calendar's zone, exclusive end; it moves onto the `AllDay` helper). The EventKit connector treats EventKit's all-day dates as floating: it takes the device-local calendar days of `startDate` and of the last covered day, and emits canonical instants with `timeZone = .current`. The last covered day is the day of `endDate`, except when `endDate` is exactly a local midnight after `startDate` (already exclusive); a zero-length event (`end <= start`) covers one day. `EKEvent.timeZone` is ignored for all-day events (floating semantics).
- **Bridge, canonical to TimeTug native.** `EventMapper` reads the year, month and day of `start` and of `end` in the event's `timeZone`, and rebuilds midnight instants of those same dates in the injected device `Calendar` (default `.autoupdatingCurrent`, so it follows time-zone changes the way the agenda's `.current` calls do). The rebuild uses the start of day of that date so a zone whose midnight does not exist on a DST day still yields a valid instant. When `timeZone` equals the device zone this is the identity, so the common case is unchanged.
...
- Generic and portable, so they live in `CalendarCore` (Foundation only, testable on Linux): `FileConnectionStore` (an actor holding `[Connection]` as JSON at an injectable URL, atomic writes, following `LedgerStore` conventions; `load()` distinguishes "no file yet" from "unreadable file" so the launch orphan sweep can refuse to run on a corrupt file) and `FileSyncStateStore: SyncStateStore` (an actor over one JSON file, injectable URL, atomic writes; a missing or unreadable file means "no tokens", so the library does a full bootstrap). TimeTug points both at Application Support (`accounts.json`, `sync-state.json`). EventKit is never stored in the connection store.
- OS-specific code lives in a separate adapter package, `Packages/CalendarApple` (Apple platforms only, Swift 5 mode like `EventKitSource`, depends on `CalendarCore` and nothing from TimeTug). Each piece implements a library protocol, so any other host can supply its own implementation instead:
  - `KeychainCredentialStore: CredentialStore`: `kSecClassGenericPassword`, service name injected (TimeTug passes `com.timetug.app.credentials`), account = `connectionID`, value = JSON of the secrets map.
  - `LoopbackAuthorizationInteraction: AuthorizationInteraction`: `beginOAuthRedirect()` starts an `NWListener` on `127.0.0.1` with an ephemeral port and returns a session whose `redirectURI` is `http://127.0.0.1:<port>`; `authorize(at:)` hands the URL to an injected `openURL` closure (TimeTug passes `NSWorkspace.shared.open`, so the adapter needs no AppKit), answers the first request with a small "You can close this window" page, and returns the received URL; it times out after 5 minutes and `close()` cancels the listener. `promptCredentials` throws unless a prompt closure is injected (unused until CalDAV).
  - `EventKitSource` remains its own adapter package on the same principle (an OS-specific connector kind and source behind the library's protocols).
- The app has no sandbox entitlement, so the listener and Keychain need no new entitlements. The app itself only wires these together and supplies TimeTug's names and paths.

**Reconciler.** A testable type, `SourceReconciler`, owns "which sources exist": given the persisted connections, the EventKit enabled flag and the registry, it builds a `[String: ConnectedSource]` keyed by source id, diffs against the running set, and reports additions and removals. `AppCoordinator` calls it at launch and after any account or toggle change, then calls `store.setSources(_:)`, starts a change-listening task for each added source (each `for await _ in source.changes() { await refresh() }`) and cancels the tasks of removed sources. The existing periodic refresh remains. The old direct EventKit access request and `eventKit.changes()` loop are replaced by this path. A failing `makeSource` for one account leaves it out of the set and records a status for the pane instead of stopping the others. `SourceReconciler` also has `rebuild(_:connections:eventKitEnabled:)` (rebuild one account's source), used after a successful re-sign-in: it builds a fresh source for the same id, replaces the old instance in the running set, and restarts that source's listener task. This is required because the library ends a source's change stream after `.sourceFailed` (expired auth), so a rebuilt source with an unchanged id would otherwise never poll again. The reconciler tracks source instances, not just ids, and a test covers "expired auth, re-sign-in, edits arrive again".
...
- `AccountsPane` and a small view model. A list with a pinned first row "Apple Calendar (this Mac)" and an enable checkbox (not removable); then one row per Google account with its email, status (connected; "Sign in again"; error text) and, for permission problems, an "Open System Settings" link. Below the list, the standard `+` and `−` controls.
  ...
  - `−` (never available for the EventKit row) asks: "Remove <email>? Its calendars will no longer appear and their Tug and visibility choices are forgotten." On confirm, in this order, so an interruption never leaves a half-removed account that looks alive: (1) delete the stored `Connection` (the record that the account exists; if this fails nothing else has changed and the user can retry); (2) stop and remove the source, which the reconciler does because it derives the source set from the stored accounts; (3) call `removeCalendars(forSourceID:)` on the persisted settings; (4) best-effort delete its sync state and Keychain secrets, reporting a Keychain failure without undoing the removal. Every step is idempotent, so repeating `−` after a crash or error is safe. As a backstop, launch runs an orphan sweep: it drops stored selection keys whose source id belongs to an account-based kind (`google-...`) but matches no stored connection. The sweep is skipped when `accounts.json` could not be read (a corrupt file must not wipe selections) and never touches `eventkit/...` keys. EventKit never goes through `removeCalendars`: the checkbox only changes the enabled flag.
...
- **Library:** one test for `needsPermission`; `AllDay` helper tests (one day, multi-day, inclusive-to-exclusive, zones with a DST-day midnight gap, round trip through `dates(start:end:in:)`); `AllDayConformance` accepts canonical events and rejects each violation; the Google mapper's all-day fixtures also run through the conformance check; the 81 existing tests keep passing.
- **Library (Swift Testing, also run on Linux):** `FileConnectionStore` round trip, missing versus unreadable file, atomic replace; `FileSyncStateStore` round trip, `removeAll`, unreadable file.
- **`CalendarApple` (XCTest):** `KeychainCredentialStore` with a per-run unique service name and cleanup; the loopback session receives a redirect on `127.0.0.1`, serves the close-window page, times out, and `close()` frees the port.
- **App (XCTest):** `SourceReconciler` add, remove, EventKit toggle, failing `makeSource`, `rebuild` restarting the listener, and identical keying by `source.id` for Google; removal order and idempotence with a store that throws at each step; the launch orphan sweep (including the unreadable-file and `eventkit/` cases); Accounts view-model add/remove/duplicate/re-sign-in using a fake `ConnectorKind` and interaction.
...
- **CI:** the `core` job also runs `swift test` for `CalendarBridge` and `CalendarApple` and builds `EventKitSource`; `connectors-linux` covers the new generic file stores; the `app` job builds and tests the app with the new package references in `project.yml`. `core-linux` is unchanged (Core does not depend on the library).
...
3. Library: `SourceError.needsPermission`, the `AllDay` helper and `AllDayConformance` (Google mapper moves onto them).
4. `CalendarBridge` package (mapper, adapter) with tests.
5. `EventKitSource` conversion and `EventKitConnectorKind`.
6. Adapters and services: `FileConnectionStore` and `FileSyncStateStore` in `CalendarCore`; the `CalendarApple` package (Keychain store, loopback authorization); app wiring and OAuth config plumbing.
